<?php
	...
	$query="SELECT DATE_FORMAT(a.fecha_asig,'%d/%m/%Y') as fecha_asig, a.cod_user, a.cod_tipo, a.codigo_asig, a.cod_depto, a.cantidad, con.cod_cons, con.costo_cons, con.depto_con, con.desc_cons, u.cod_user, u.nombre_user, d.desc_depto, a.cantidad*con.costo_cons as multi FROM asignaciones a LEFT JOIN consumibles con ON con.cod_cons = a.codigo_asig LEFT JOIN usuarios u ON a.cod_user = u.cod_user  LEFT JOIN departamentos d ON a.cod_depto = d.cod_depto WHERE a.cod_tipo=1 AND a.cod_user IS NOT NULL $cond1 $cond ORDER BY a.cod_user LIMIT $offset,$rows";
